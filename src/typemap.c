...
jl_typemap_entry_t *jl_typemap_morespecific_by_type(jl_typemap_entry_t *first, jl_value_t *types, jl_svec_t **penv, size_t world)
{
    if ((jl_value_t*)first->prev == jl_nothing)
        return first;
    int isambig = 0;
    jl_typemap_entry_t *candidate = first;
    jl_array_t *prev = first->prev;
    size_t i, l = jl_array_len(prev);
    // pick a method out of the list that is more specific than any other applicable method
    for (i = 0; i < l; i++) {
        jl_typemap_entry_t *prior = (jl_typemap_entry_t*)jl_array_ptr_ref(prev, i);
        if (prior->min_world <= world && world <= prior->max_world && jl_subtype(types, (jl_value_t*)prior->sig)) {
            if (jl_type_morespecific((jl_value_t*)prior->sig, (jl_value_t*)candidate->sig)) {
                candidate = prior;
                isambig = 0; // reset to unsure
            }
    ...
    if (candidate == first)
        return candidate;
    // and then make sure it is more specific than all other applicable methods
    for (i = 0; i < l; i++) {
        jl_typemap_entry_t *prior = (jl_typemap_entry_t*)jl_array_ptr_ref(prev, i);
        if (prior == candidate)
                                      ...
                                      size_t min_world, size_t max_world)
{
    ...
    newrec->prev = (jl_array_t*)jl_nothing;
